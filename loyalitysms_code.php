...
$sql2 = "SELECT *  FROM `tbl_user_group` WHERE `store_id` = '$store'";
//echo $sql2;
...
$sql4 = "SELECT *  FROM `tbl_customer_data` WHERE `id` = '$id'";
//echo $sql4;
...
$sql12 = "SELECT *  FROM `tbl_user_data` WHERE `store` = '$store' and `roles_name` = 'superadmin'";
...
$business_name=$row12['business_name'];
 ...
 $sql001 = "SELECT *  FROM `smstemplates` WHERE `message_type` = 'Loyality' and `store` = '$store'";
...
if(!empty($mess)){
	 ...
	 $sql002 = "SELECT *  FROM `smstemplates` WHERE `message_type` = 'Loyality' and `store` = '0'";
...
if($response=='sms sent successfully'){
	...
	header('Location: loyality.php?data=update');
	exit;
	}
	else
	{
	header('Location: loyality.php?data=error');
	exit;
	}
}else{
	header('Location: loyality.php?data=error');
